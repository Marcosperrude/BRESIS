...
fatdesEPE = df.div(df['2022'], axis=0)

fatdesEPE.index = fatdesEPE.index.str.strip()

fatores_lenha_carvao = fatdesEPE.loc[['LENHA', 'CARVÃO VEGETAL']].rename(index={
    'LENHA': 'Lenha',
    'CARVÃO VEGETAL': 'Carvao'
})

fatores_lenha_carvao.to_csv(os.path.join(DataPath, 'fatdesEPE.csv'))
